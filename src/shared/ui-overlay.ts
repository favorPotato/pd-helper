type Platform = 'tiktok' | 'instagram' | 'nox' | 'runtime' | 'exolyt' | 'unknown';
...
export class FixedOverlay {
    ...
    private render() {
      ...
      .status.exolyt { background: linear-gradient(135deg, #f0277e, #de1a6b); color: #fff; }
      ...
      :host([data-pd-helper-collapsed="true"][data-pd-helper-platform="exolyt"]) .card {
        background: linear-gradient(135deg, #f0277e, #de1a6b);
        box-shadow: 0 6px 18px rgba(230, 30, 115, 0.35);
      }
    ...
    private getCollapsedLabel(): string {
        ...
        if (this.platform === 'exolyt') return 'EX';
